...
func (t *httpTransport) sendTraces(traces [][]*span) (*http.Response, error) {
	...
	req, err := http.NewRequest("POST", t.traceURL, encodedReader(t.encoding, traces))
	if err != nil {
...
func (t *httpTransport) sendServices(services map[string]service) (*http.Response, error) {
	...
	// Send it
	req, err := http.NewRequest("POST", t.serviceURL, encodedReader(t.encoding, services))
	if err != nil {
		return nil, fmt.Errorf("cannot create http request: %v", err)
	}
	for header, value := range t.headers {
	...
	req.Header.Set("Content-Type", string(t.encoding.contentType()))
